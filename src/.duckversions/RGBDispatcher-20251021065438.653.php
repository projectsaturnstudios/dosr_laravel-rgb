...
class RGBDispatcher
{
    ...
    public function __call(string $method, array $arguments): mixed
    {
        ...
        // If async mode is enabled, queue the operation
        if ($this->use_async) {
            ...
            function() use ($channel, $queue) {
                foreach ($queue as $operation) {
                    if ($operation['type'] === 'wait') {
                        ...
                        $channel->{$operation['method']}(...$operation['args']);
                    }
                }
            }
        ]);
    ...
    public function status(string $status, int $blinks = 2): static
    {
        ...
        if ($this->use_async) {
            for ($i = 0; $i < $blinks; $i++) {
                $this->operation_queue[] = ['type' => 'method', 'method' => 'fill', 'args' => [$color]];
                $this->operation_queue[] = ['type' => 'method', 'method' => 'show', 'args' => []];
                $this->operation_queue[] = ['type' => 'wait', 'duration' => 150];
                $this->operation_queue[] = ['type' => 'method', 'method' => 'clear', 'args' => []];
                $this->operation_queue[] = ['type' => 'method', 'method' => 'show', 'args' => []];
                if ($i < $blinks - 1) {
                    $this->operation_queue[] = ['type' => 'wait', 'duration' => 150];
                }
