...
class HighStriker extends AnimatablePixelBus
{
    ...
    /**
     * Flip the rail strip direction (useful if mounted upside down)
     * Reverses the pixel order so pixel 0 becomes last pixel and vice versa
     *
     * @return static
     */
    public function flip(): static
    {
        ...
        if (method_exists($rail, 'flip')) {
            // If RGBStrip has a native flip method, use it
            $rail->flip();
        } else {
            // Otherwise, set a reverse flag on the rail if it's an RGBStrip
            if ($rail instanceof RGBStrip && property_exists($rail, 'reverse_pixels')) {
                $rail->reverse_pixels = !$rail->reverse_pixels;
            }
        }
